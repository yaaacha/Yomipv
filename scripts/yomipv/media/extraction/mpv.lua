...
local function build_base_command(source, ...)
	local args = {
		...
		"--no-ocopy-metadata",
		"--hr-seek=yes",
		"--ytdl-raw-options-add=user-agent=Mozilla/5.0",
		"--cache=yes",
	...
	for i = 1, select("#", ...) do
		local arg = select(i, ...)
		if arg ~= nil then table.insert(args, arg) end
	end
...
function MpvEncoder.generate_picture_args(config, source, output, time, duration)
	local codec_flags
	local timeline_flags = { "--start=" .. MediaUtils.to_timestamp_str(time) }
	...
	if format == "avif" then
		...
		if not config.picture_animated then table.insert(codec_flags, "--ovcopts-add=still-picture=1") end
	elseif format == "webp" then
		codec_flags = {
			...
			string.format("--ovcopts-add=lossless=%d", config.picture_webp_lossless and 1 or 0),
		}
	else
		-- MJPEG (Default)
		codec_flags = {
...
-- Audio Extraction
function MpvEncoder.generate_audio_args(config, source, output, start_time, end_time, volume, aid_override)
	local audio_flags
	...
	local aid = aid_override or mp.get_property("aid") or "1"
