...
function Audio.create_job(subtitle)
    ...
    -- Padding settings
    local padding_start = Audio.config.audio_padding_start or 0.1
    local padding_end = Audio.config.audio_padding_end or 0.3

    local start_time = math.max(0, (subtitle.start or 0) - padding_start)
    local end_time = (subtitle["end"] or 0) + padding_end
    ...
    local timestamp_str = MediaUtils.to_timestamp_str(mp.get_property_number("time-pos") or 0)

    local extension = Audio.config.audio_format or "mp3"
    local filename = MediaUtils.generate_filename("yomipv_audio", extension, Audio.config.filename_show_ms)
    local target_file = utils.join_path(Audio.output_dir, filename)
    
    local misc_info = string.format("%s | Ep: %s | %s [%s]", anime_title, ep_num, ep_title, timestamp_str)
    ...
    function job:run()
        ...
        mp.command_native_async({
            ...
            if success and result.status == 0 then
                ...
                msg.error("Audio extraction failed. Status: " .. tostring(status))
